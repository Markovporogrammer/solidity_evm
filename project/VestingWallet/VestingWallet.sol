    ...
    function vestedAmount(address token, uint256 timestamp) public view returns (uint256) {
        ...
        if (timestamp < start) {
            ...
            return 0;
             ...
             console.log("timestamp > start + duration", start + duration);
            return totalAllocation;
        } else {
            console.log("other", totalAllocation, timestamp , start);
            // 合约总数代币数量乘以一个每次取钱调用时间差和归属期的比值确定在每一个不超过归属其的时间段不能一次性全部取出
